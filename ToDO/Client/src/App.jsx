...
function App() {
  ...
  let handleEdit = async (id) => {
    ...
    try {
      let res = await fetch(`http://localhost:3001/edittodo/${id}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ message: updateTodo }),
      });
      if (!res.ok) {
        throw new Error("Failed to update the to-do item");
      }

      console.log(`To-Do with id ${id} updated successfully.`);
    } catch (error) {
      console.log(error);
    }
